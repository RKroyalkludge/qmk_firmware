...
void bat_indicators_hook(uint8_t index) {
    ...
    if (charging_state && ( (bat_full_flag))) { 
        ...
        bat_flip             = true;
        rgb_matrix_blink_set_color(index, 0x00, 0x0F, 0x00);
        rgb_matrix_blink_set_interval_times(index, 300, 0xFF); 
    } else if (charging_state) {                         
        battery_process_time = 0;
        bat_flip             = true;
        rgb_matrix_blink_set_color(index, 0x0F, 0x00, 0x00);
        rgb_matrix_blink_set_interval_times(index, 300, 0xFF);  
...
void ind_indicators_hook(uint8_t index) {
    ...
    switch(ind_status) {
        case ind_state_none: {
            rgb_matrix_blink_set_color(index, host_keyboard_led_state().caps_lock,host_keyboard_led_state().num_lock, 0x00);
...
bool rgb_matrix_blink_user(blink_rgb_t *blink_rgb) {
    ...
    switch (blink_rgb->index) {
        case RGB_MATRIX_BLINK_INDEX_BAT: {
            ...
            if (blink_rgb->flip) {
                rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BAT, blink_rgb->color.r, blink_rgb->color.g, blink_rgb->color.b);
            } else {
                rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BAT, 0x00, 0x00, 0x00);
            }
        ...
        case RGB_MATRIX_BLINK_INDEX_IND: {
            if (blink_rgb->flip) {
                if (blink_rgb->color.r) {
                    rgb_matrix_set_color(HS_RGB_INDEX_INDEX_CAP, rgb_caps_open.r+2, rgb_caps_open.g, rgb_caps_open.b);
                } else {
                    rgb_matrix_set_color(HS_RGB_INDEX_INDEX_CAP, 0x00, 0x00, 0x00);
                }

                if (blink_rgb->color.g) {
                    rgb_matrix_set_color(HS_RGB_INDEX_INDEX_NUM, rgb_num_open.r, rgb_num_open.g, rgb_num_open.b);
                } else {
                    rgb_matrix_set_color(HS_RGB_INDEX_INDEX_NUM, 0x00, 0x00, 0x00);
                }
                if ((ind_status != ind_state_none)) {
                    if ((blink_rgb->color.b)) {
                        rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BAT, rgb_ind_open.r, rgb_ind_open.g, rgb_ind_open.b);
                    } else {
                        rgb_matrix_set_color(HS_RGB_BLINK_INDEX_BAT, 0x00, 0x00, 0x00);
                    }
                }
...
void keyboard_post_init_kb(void) {
    ...
    rgb_ind_open = hsv_to_rgb((HSV){.h = 4, .s = 80, .v = (RGB_MATRIX_VAL_STEP*3)});
    rgb_num_open = hsv_to_rgb((HSV){.h = 4, .s = 80, .v = 110});
    rgb_caps_open = hsv_to_rgb((HSV){.h = 4, .s = 80, .v = 120});
    rgb_bat_open = hsv_to_rgb((HSV){.h = 0, .s = 255, .v = RGB_MATRIX_DEFAULT_VAL}); 
}
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    ...
    if (!rgbrec_is_started()) {
        ...
        if (!keymap_is_mac_system() && keymap_config.no_gui) {
            rgb_matrix_set_color(HS_RGB_INDEX_WIN_LOCK, 200, 200, 200);
        }   
    }
