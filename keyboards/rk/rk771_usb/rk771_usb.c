...
void bat_indicators_hook(uint8_t index) {
    ...
    } else if (charging_state && (!bat_flag)) {                        
        battery_process_time = 0;
        bat_flip             = true;
        ...
        case RGB_MATRIX_BLINK_INDEX_IND: {
            ...
            switch(ind_status) {
                ...
                case ind_state_exec: {
                    ...
                    if (blink_rgb->flip) {
                        rgb_matrix_set_color(0, 0X14, 0X14, 0X14);
                        rgb_matrix_set_color(1, 0X14, 0X14, 0X14);
                        rgb_matrix_set_color(2, 0X14, 0X14, 0X14);
                    } else {
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    ...
    if (!rgbrec_is_started()) {
        if (host_keyboard_led_state().caps_lock)
            rgb_matrix_set_color(HS_RGB_INDEX_INDEX_CAP, 0x14, 0x14, 0x14);
        else 
            rgb_matrix_set_color(HS_RGB_INDEX_INDEX_CAP, 0x00, 0x00, 0x00);

        if (!keymap_is_mac_system() && keymap_config.no_gui)
            rgb_matrix_set_color(HS_RGB_INDEX_WIN_LOCK, 0x14, 0x14, 0x14);
        else
