...
blink_rgb_t blink_rgbs[RGB_MATRIX_BLINK_COUNT] = {
    ...
    {.index = IM_MM_RGB_BLINK_INDEX_DEB, .interval = 250, .times = 3, .color = {.r = WIITE_B, .g = WIITE_B, .b = WIITE_B}, .blink_cb = NULL},
...
void bat_indicators_hook(uint8_t index) {
    ...
    if (charging_state && ( (bat_full_flag))) { 
        ...
        rgb_matrix_blink_set_color(index, 0x00, 0x51, 0x00);
        rgb_matrix_blink_set_interval_times(index, 200, 0xFF); // 灭灯
    } else if (charging_state) {                        
        battery_process_time = 0;
        bat_flip             = true;
        
        rgb_matrix_blink_set_color(index, 0x51, 0x00, 0x00);
        rgb_matrix_blink_set_interval_times(index, 200, 0xFF); // 红灯常亮
    } else if (*md_getp_bat() <= BATTERY_CAPACITY_LOW) {
        if (bat_flip) {
            rgb_matrix_blink_set_color(index, 0x51, 0x00, 0x00);
            rgb_matrix_blink_set_interval_times(index, 300, 1); 
        } else {
...
bool rgb_matrix_blink_user(blink_rgb_t *blink_rgb) {
    ...
    switch (blink_rgb->index) {
        case RGB_MATRIX_BLINK_INDEX_BAT: {
            ...
            if (blink_rgb->flip) {
                rgb_matrix_set_color(2, blink_rgb->color.r, blink_rgb->color.g, blink_rgb->color.b);
            } else {
        ...
        case RGB_MATRIX_BLINK_INDEX_IND: {
            ...
            switch(ind_status) {
                ...
                case ind_state_blinkoff: {
                    if (blink_rgb->flip) {
                        rgb_matrix_set_color(0, 0X51, 0X51, 0X51);
                        rgb_matrix_set_color(1, 0X51, 0X51, 0X51);
                    } else {
                ...
                case ind_state_exec: {
                    ...
                    if (blink_rgb->flip) {
                        rgb_matrix_set_color(HS_RGB_BLINK_INDEX_VAI,0x51,0x51,0x51);
                        rgb_matrix_set_color(HS_RGB_BLINK_INDEX_VAD,0x51,0x51,0x51);
                        rgb_matrix_set_color(HS_RGB_BLINK_INDEX_SPI,0x51,0x51,0x51);
                        rgb_matrix_set_color(HS_RGB_BLINK_INDEX_SPD,0x51,0x51,0x51);
                    } else {
    ...
    #        endif
            ...
            if (record->event.pressed) {
                if (rgb_matrix_get_mode() == RGB_MATRIX_CUSTOM_RGBR_PLAY) {
                    if (rgbrec_is_started()) {
                        ...
                        rgbrec_end(confinfo.record_channel,false);
                        no_record_fg = false;
                        if (ind_status == ind_state_blinkon) ind_set_blink(ind_state_none);
                    }
        ...
        case DEB_TOG:{ 
            if (record->event.pressed) {
                ...
                rgb_matrix_blink_set_interval_times(IM_MM_RGB_BLINK_INDEX_DEB, 250, 1);
                rgb_matrix_blink_set(IM_MM_RGB_BLINK_INDEX_DEB);
                if(!confinfo.DEB_flag){
                    hs_deb = DEBOUNCE; 
                }else{
                    hs_deb = 1; 
...
void housekeeping_task_user(void) { // loop
    ...
    if (ind_status != ind_state_blinkon){
...
void rgb_matrix_wls_indicator(void) {
    ...
    if (wls_rgb_indicator_timer) {
        ...
        if (wls_rgb_indicator_times % 2) {
            ...
            rgb_matrix_set_color(0, wls_rgb_indicator_rgb.g, wls_rgb_indicator_rgb.r, wls_rgb_indicator_rgb.b);
#else
            ...
            rgb_matrix_set_color(0x00, 0x00, 0x00, 0x00);
        }
    }
}
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    ...
    if (!rgbrec_is_started()) {
        ...
        if (!keymap_is_mac_system() && keymap_config.no_gui)
            rgb_matrix_set_color(HS_RGB_INDEX_WIN_LOCK, 0x51, 0x51, 0x51);
    }
