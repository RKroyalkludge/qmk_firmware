...
void bat_indicators_hook(uint8_t index) {
    ...
    if (charging_state && ( (bat_full_flag))) { 
        ...
        if (bat_status != 2){
            rgb_matrix_blink_set_color(0xFF, 0x01, 0x50, 0x01);
            rgb_matrix_blink_set_interval_times(0xFF, 200, 3); // 灭灯
            rgb_matrix_blink_set(0xFF);
        }
        bat_status = 2;
        ...
        rgb_matrix_blink_set_color(index, 0x50, 0x00, 0x00);
        rgb_matrix_blink_set_interval_times(index, 300, 0xFF); // 红灯常亮
        if (bat_status != charging_state){
            rgb_matrix_blink_set_color(0xFF, 0x00, 0x00, 0x50);
            rgb_matrix_blink_set_interval_times(0xFF, 200, 3); // 灭灯
            rgb_matrix_blink_set(0xFF);
        }
        bat_status = 1;
...
bool rgb_matrix_blink_user(blink_rgb_t *blink_rgb) {
    ...
    switch (blink_rgb->index) {
        case RGB_MATRIX_BLINK_INDEX_BAT: {
            ...
            if (rgbrec_is_started() || charging_state) {
                return false;
            }

            if(blink_rgb->color.r == 0x00 && blink_rgb->color.g == 0x00 && blink_rgb->color.b == 0x00) {
                return false;
            }

            if (blink_rgb->flip) {
                rgb_matrix_set_color(7, blink_rgb->color.r, blink_rgb->color.g, blink_rgb->color.b);
            } else {
        ...
        case RGB_MATRIX_BLINK_INDEX_IND: {
            ...
            switch(ind_status) {
                ...
                case ind_state_exec: {
                    ...
                    if (blink_rgb->flip) {
                        rgb_matrix_set_color_all(0x20, 0x20, 0x20);
                    } else {
...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case MM_DIR:{
            if (record->event.pressed){
                confinfo.mm_dir = !confinfo.mm_dir;
                eeconfig_confinfo_update();
            }
            return false;
        }break;
        ...
        case MM_FC:{
            if (record->event.pressed){
                confinfo.ntf++;
                if (confinfo.ntf > 2) confinfo.ntf = 0;
                eeconfig_confinfo_update();
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    ...
    if (!rgbrec_is_started()) {
        if (host_keyboard_led_state().caps_lock)
            rgb_matrix_set_color(HS_RGB_INDEX_INDEX_CAP, 0x50, 0x50, 0x50);
        else 
            rgb_matrix_set_color(HS_RGB_INDEX_INDEX_CAP, 0x00, 0x00, 0x00);

        if (!keymap_is_mac_system() && keymap_config.no_gui)
            rgb_matrix_set_color(HS_RGB_INDEX_WIN_LOCK, 0x50, 0x50, 0x50);
        else
            rgb_matrix_set_color(HS_RGB_INDEX_WIN_LOCK, 0x00, 0x00, 0x00);
    }
