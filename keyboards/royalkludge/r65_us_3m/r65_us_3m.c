...
static bool light_low_level  = false;
static uint32_t hs_vad_time  = 0;
uint32_t hs_suspend_timer    = 0;
uint16_t hs_deb              = DEBOUNCE;
bool win_mac_flag            = false;
bool ch_led_flag             = false; 
...
void bat_indicators_hook(uint8_t index) {
    ...
    if (charging_state && ((bat_full_flag))) { 
        ...
        if (bat_flip) {
            rgb_matrix_blink_set_color(index, 0xFF, 0x00, 0x00);
            rgb_matrix_blink_set_interval_times(index, 250, 1); 
        } else {
            rgb_matrix_blink_set_color(index, 0x00, 0x00, 0x00);
...
bool rgb_matrix_blink_user(blink_rgb_t *blink_rgb) {
    ...
    switch (blink_rgb->index) {
        case RGB_MATRIX_BLINK_INDEX_BAT: {
            ...
            if (blink_rgb->flip) {
                if (ch_led_flag == true) {
                    writePin(LED_CHARING_PIN, !LED_PIN_ON_STATE);
                } else {
                    writePin(LED_CHARING_PIN, blink_rgb->color.r ? LED_PIN_ON_STATE : !LED_PIN_ON_STATE);
                }
        ...
        case RGB_MATRIX_BLINK_INDEX_IND: {
            if (blink_rgb->flip) {
                writePin(LED_CAPS_PIN, blink_rgb->color.r ? LED_PIN_ON_STATE : !LED_PIN_ON_STATE);
                writePin(LED_WIN_PIN, blink_rgb->color.g ? LED_PIN_ON_STATE : !LED_PIN_ON_STATE);
                if(win_mac_flag == false){
                    if(ind_status != ind_state_none)
                        writePin(LED_CHARING_PIN, blink_rgb->color.b ? LED_PIN_ON_STATE : !LED_PIN_ON_STATE);
                }
...
bool mm_get_rgb_enable(void) {
    return confinfo.hs_rgb_tog_flag;
}

// 此函数不需要改动
void mm_set_rgb_enable(bool state) {
    confinfo.hs_rgb_tog_flag = state;
...
#ifdef ENCODERS_ENABLE_PIN
    gpio_set_pin_output(ENCODERS_ENABLE_PIN);
    gpio_write_pin_low(ENCODERS_ENABLE_PIN);
#endif
...
#ifdef ENCODERS_ENABLE_PIN
    gpio_write_pin_high(ENCODERS_ENABLE_PIN);
#endif
...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case RGB_VAI: {
            if (rgbrec_is_started()) return false;
            if (record->event.pressed) {
                light_low_level = false;
                hs_vad_time     = timer_read32();
                if (!readPin(LED_POWER_EN_PIN)) {
                    gpio_write_pin_high(LED_POWER_EN_PIN);
                    return false;
                } else {
                    start_hsv = rgb_matrix_get_hsv();
        ...
        case HS_BR: {
            if (record->event.pressed) {
                ...
                register_code16(0x0087);
            } else {
                unregister_code16(KC_RSFT);
                unregister_code16(0x0087);
            }
            return false;
        } break;
        case L_TOG: {
            if (record->event.pressed) {
                confinfo.layer_tog_flag = !confinfo.layer_tog_flag;
                eeconfig_confinfo_update();
            } 
            return false;
        } break;
        case KC_1: {
            if (confinfo.layer_tog_flag) { 
                if (record->event.pressed) {
                    register_code(KC_F1);
                } else {
        ...
        case KC_F1: {
            if (confinfo.layer_tog_flag) { 
                if (keymap_is_mac_system()) {
                    ...
                    if (record->event.pressed) {
                        register_code(KC_MYCM);
                    } else {
                        unregister_code(KC_MYCM);
                    }
                }
                return false; 
            }
        ...
        case KC_F2: {
            if (confinfo.layer_tog_flag) { 
                if (keymap_is_mac_system()) {
                    ...
                    if (record->event.pressed) {
                        register_code(KC_WHOM);
                    } else {
                        unregister_code(KC_WHOM);
                    }
                }
                return false; 
            }
        ...
        case KC_F3: {
            if (confinfo.layer_tog_flag) { 
                if (keymap_is_mac_system()) {
                    ...
                    if (record->event.pressed) {
                        register_code(KC_MAIL);
                    } else {
                        unregister_code(KC_MAIL);
                    }
                }
                return false; 
            }
        ...
        case KC_F5: {
            if (confinfo.layer_tog_flag) { 
                if (keymap_is_mac_system()) {
                    ...
                    if (record->event.pressed) {
                        register_code(KC_MSEL);
                    } else {
                        unregister_code(KC_MSEL);
                    }
                }
                return false; 
            }
...
void hs_rgb_power_swicth(void) {
    if (light_low_level == true) {
        if ((timer_elapsed32(hs_vad_time) > 3000)) {
            light_low_level = false;
            hs_vad_time = timer_read32();
            gpio_write_pin_low(LED_POWER_EN_PIN);
        }
    ...
    if (wireless_get_current_devs() == DEVS_USB) { 
        ...
        switch (*md_getp_state()) {
            ...
            case MD_STATE_CONNECTED: {
                if(linker_ok == false){
                    if((rgb_matrix_get_val() <= 0) || (!readPin(LED_POWER_EN_PIN))){
                        light_low_level = true;
                        hs_vad_time = timer_read32();
                        gpio_write_pin_high(LED_POWER_EN_PIN);
                    }  
