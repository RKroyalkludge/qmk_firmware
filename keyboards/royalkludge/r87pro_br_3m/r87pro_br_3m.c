...
void bat_indicators_hook(uint8_t index) {
    ...
    static uint8_t flashing_blue_cnt_3   = 0; 
    static uint8_t flashing_green_cnt_3  = 0; 
    ...
    if (charging_state && ( (bat_full_flag))) { 
        ...
        flashing_green_cnt_3 = 0;
        full_cnt = timer_read32();
        if(flashing_blue_cnt_3 >= 3){
            rgb_matrix_blink_set_color(index, 0x00, 0x00, 0x00);
            rgb_matrix_blink_set_interval_times(index, 300, 0xFF); 
        }else{
            flashing_blue_cnt_3 ++ ;
            rgb_matrix_blink_set_color(index, 0x00, 0x00, 0xFF);
            rgb_matrix_blink_set_interval_times(index, 500, 1);  
        }
        ...
        case RGB_MATRIX_BLINK_INDEX_BAT: {
            ...
            if (blink_rgb->flip) {
                    if(blink_rgb->color.r){
                        for (uint8_t i = RGB_MATRIX_INDEX_BAT_START; i < RGB_MATRIX_INDEX_BAT_END; i++) {
                            rgb_matrix_set_color(i, 0xff, 0x00, 0x00);
                        }
                    }else if(blink_rgb->color.g){
                        for (uint8_t i = RGB_MATRIX_INDEX_BAT_START; i < RGB_MATRIX_INDEX_BAT_END; i++) {
                            rgb_matrix_set_color(i, 0x00, 0xff, 0x00);
                        }                        
                    }else if(blink_rgb->color.b){
                        for (uint8_t i = RGB_MATRIX_INDEX_BAT_START; i < RGB_MATRIX_INDEX_BAT_END; i++) {
