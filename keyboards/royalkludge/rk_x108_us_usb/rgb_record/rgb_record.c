...
//clang-format off
static const uint8_t rgbmatrix_buff[] = {13, 15, 16, 26, 29, 30, 6, 9 ,24 ,17};
static const uint8_t interactive_buff[] = {35, 40, 43, 34, 33};
static const uint8_t breathing_buff[] = {5};
static const uint8_t cr_breathing_buff[] = {25};
static const uint8_t rgbmatrix_buff_all[] = {
    13, 15, 16, 26, 29, 30, 6, 9 ,24 ,17,  //10种动态背光模式
    35, 40, 43, 34, 33,                    //5种互动模式
    5,                                     //单色呼吸模式
    25                                     //多彩呼吸模式
    };
...
// 清除全部记忆的灯效HS
void all_color_reset(void) {
    for (uint32_t i = 0; i < sizeof(rgbmatrix_buff_all) / sizeof(rgbmatrix_buff_all[0]); i++) {
        eeprom_write_byte((uint8_t *)(((uint32_t)CONFINFO_EECONFIG_ADDR + 4) +  i), 0x00);
    }
}

uint8_t find_index(void) {

    for (uint8_t index = 0; index < (sizeof(rgbmatrix_buff_all) / sizeof(rgbmatrix_buff_all[0])); index++) {
        if (rgb_matrix_get_mode() == rgbmatrix_buff_all[index]) {
...
void record_rgbmatrix_increase(uint8_t *last_mode) {
    ...
    index = *last_mode;//find_index();
    if (rgbrec_info.state != RGBREC_STATE_ON) {
        index = (index + 1) % (sizeof(rgbmatrix_buff) / sizeof(rgbmatrix_buff[0]));
    }
    *last_mode = index;//rgbmatrix_buff[index];
...
void record_interactive_mod(uint8_t *last_mode) {
    uint8_t index;

    index = *last_mode;//find_index();
    if (rgbrec_info.state != RGBREC_STATE_ON) {
        index = (index + 1) % (sizeof(interactive_buff) / sizeof(interactive_buff[0]));
    }
    *last_mode = index;//interactive_buff[index];
    rgb_matrix_mode(interactive_buff[index]);
    //record_color_hsv(false);
    uint8_t rgb_hsv_index =  record_color_read_data();
    rgb_matrix_sethsv(rgb_hsvs[rgb_hsv_index][0], rgb_hsvs[rgb_hsv_index][1], rgb_matrix_get_val());
}

void record_breathing_mod(uint8_t *last_mode) {
    uint8_t index;

    index = *last_mode;//find_index();
    if (rgbrec_info.state != RGBREC_STATE_ON) {
        index = (index + 1) % (sizeof(breathing_buff) / sizeof(breathing_buff[0]));
    }
    *last_mode = index;//breathing_buff[index];
    rgb_matrix_mode(breathing_buff[index]);
    //record_color_hsv(false);
    uint8_t rgb_hsv_index =  record_color_read_data();
    rgb_matrix_sethsv(rgb_hsvs[rgb_hsv_index][0], rgb_hsvs[rgb_hsv_index][1], rgb_matrix_get_val());
}

void record_cr_breathing_mod(uint8_t *last_mode) {
    uint8_t index;

    index = *last_mode;//find_index();
    if (rgbrec_info.state != RGBREC_STATE_ON) {
        index = (index + 1) % (sizeof(cr_breathing_buff) / sizeof(cr_breathing_buff[0]));
    }
    *last_mode = index;//cr_breathing_buff[index];
    rgb_matrix_mode(cr_breathing_buff[index]);
    //record_color_hsv(false);
    uint8_t rgb_hsv_index =  record_color_read_data();
    rgb_matrix_sethsv(rgb_hsvs[rgb_hsv_index][0], rgb_hsvs[rgb_hsv_index][1], rgb_matrix_get_val());
}
