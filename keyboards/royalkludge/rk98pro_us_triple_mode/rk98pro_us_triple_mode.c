...
void bat_indicators_hook(uint8_t index) {
    ...
    if (charging_state && ( (bat_full_flag))) { 
        ...
        // bat_flip             = true;
        bat_char             = false;
        writePin(LED_POV_PIN, 1);
        // rgb_matrix_blink_set_color(index, 0x19, 0x00, 0x00);
        // rgb_matrix_blink_set_interval_times(index, 200, 0xFF); 
    } else if (charging_state) {                        
        battery_process_time = 0;
        bat_char             = false;
        // bat_flip             = true;
        writePin(LED_POV_PIN, 0);
        // rgb_matrix_blink_set_color(index, 0, 0x19, 0x00);
        // rgb_matrix_blink_set_interval_times(index, 200, 0xFF); 
    } else if (*md_getp_bat() <= BATTERY_CAPACITY_LOW) {
        bat_char = true;
        writePin(LED_POV_PIN, 1);
        if(laste_time==0)laste_time=timer_read();
        if(timer_elapsed(laste_time)>500){
            laste_time=timer_read();
            writePin(LED_WIN_PIN, !readPin(LED_WIN_PIN));
        }
        // if (bat_flip) {
        //     rgb_matrix_blink_set_color(index, 0x19, 0x00, 0x00);
        //     rgb_matrix_blink_set_interval_times(index, 1000, 1); 
        // } else {
            ...
            switch(ind_status) {
                ...
                case ind_state_exec: {
                    ...
                    if (blink_rgb->flip) {
                        ...
                        writePin(LED_CAPS_PIN, 1);
                        writePin(LED_SCROLL_PIN, 1);
                        writePin(LED_NUM_PIN, 1);
                    } else {
                        writePin(LED_WIN_PIN, 0);
                        writePin(LED_CAPS_PIN, 0);
                        writePin(LED_SCROLL_PIN, 0);
                        writePin(LED_NUM_PIN, 0);
                    }
                } break;
                default: break;
            }
...
void keyboard_post_init_kb(void) {
    ...
    gpio_set_pin_output(LED_SCROLL_PIN);
#endif
...
#ifdef LED_POV_PIN
    gpio_set_pin_output(LED_POV_PIN);
#endif
...
#ifdef LED_SCROLL_PIN
    gpio_write_pin_high(LED_SCROLL_PIN);
#endif

#ifdef LED_POV_PIN
    gpio_write_pin_high(LED_POV_PIN);
#endif
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    ...
    if (!rgbrec_is_started() && ind_status == ind_state_none) {
        if (host_keyboard_led_state().caps_lock)
            writePin(LED_CAPS_PIN, LED_PIN_ON_STATE);
        else 
            writePin(LED_CAPS_PIN, !LED_PIN_ON_STATE);

        if (!keymap_is_mac_system() && keymap_config.no_gui && !bat_char)
            writePin(LED_WIN_PIN, LED_PIN_ON_STATE);
        else if (!bat_char)
            writePin(LED_WIN_PIN, !LED_PIN_ON_STATE);

        if (host_keyboard_led_state().num_lock)
            writePin(LED_NUM_PIN, LED_PIN_ON_STATE);
        else 
            writePin(LED_NUM_PIN, !LED_PIN_ON_STATE);

        if (host_keyboard_led_state().scroll_lock)
            writePin(LED_SCROLL_PIN, LED_PIN_ON_STATE);
        else 
            writePin(LED_SCROLL_PIN, !LED_PIN_ON_STATE);
    }
